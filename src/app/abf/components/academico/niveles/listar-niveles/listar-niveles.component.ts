...
import { NivelesService } from 'src/app/abf/service';
...
saveItem() {
  if(this.item.idNivel){
    this.item.userModificacion='admin'

    this.service.modificar(this.item, this.item.idNivel).subscribe( data =>{
...
mostrarDatos(item:any, props:any){
  ...
  buscar(even:any){
    ...
    let params ={
        ...
        orderBy:sortField?sortField:"idNivel",
